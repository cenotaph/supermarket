...
class Video < ActiveRecord::Base
  ...
  def populate_other_fields
    if in_url =~ /youtube/ || in_url =~ /youtu\.be/
      ...
      self.provider_id = in_url.match(%r{^.*((youtu.be/)|(v/)|(/u/\w/)|(embed/)|(watch\?))\??v?=?([^#\&\?]*).*})[7] || 'error'
      self.videohost_id = 1
      v = client.video_by(provider_id)
      self.title = v.title
      self.description = v.description
      self.remote_thumbnail_url = begin
                                    v.thumbnails.max_by(&:width).url
                                  rescue StandardError
                                    nil
                                  end
      self.duration = v.duration
    elsif /vimeo/.match?(in_url)
      self.provider_id = in_url.match(%r{^\D*/(\d*).*})[1] || 'error'
      self.videohost_id = 2
